...
do_join(State, Ref, ChatName) ->
    	IsIn = maps:find(ChatName, State#cl_st.con_ch),
		if IsIn =/= error ->
			whereis(list_to_atom(State#cl_st.gui))!{result, self(), Ref, err};
		true ->
			whereis(server)!{self(), Ref, join, ChatName},
			receive {_From, Ref, connect, History} ->
				maps:put(ChatName, self(), State#cl_st.con_ch),
...
do_leave(State, Ref, ChatName) ->
		...
		if IsIn == error ->
			...
			true ->
				...
				receive {_From, Ref, ack_leave} ->
					maps:remove(ChatName, State#cl_st.con_ch),
...
do_msg_send(State, Ref, ChatName, Message) ->
    {ok, C_ID} = maps:find(ChatName, State#cl_st.con_ch),
	C_ID!{self(), Ref, message, Message},
	receive {_From, Ref, ack_msg} -> 
